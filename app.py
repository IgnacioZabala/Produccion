...
try:
    ...
    with st.sidebar.container():
        ...
        # Recuadro dinámico para ingreso manual si se selecciona Mastellone
        litros_manual_mastellone = 0.0
        if filtro_grupo == "Mastellone":
            st.markdown("---")
            st.markdown("### 📥 Ingreso Manual")
            litros_manual_mastellone = st.number_input(
                "Litros Ingresados (Mastellone)", 
                min_value=0.0, 
                value=617294.0, 
                step=100.0,
                format="%.2f"
    ...
    # DataFrame Consolidado (Única Vista)
    ...
    total_litros_ingresados_sistema = df_recibo_filtrado['Litros Ingresados'].sum()

    # Si se selecciona Mastellone, usamos el valor manual ingresado
    if filtro_grupo == "Mastellone":
        total_litros_ingresados = litros_manual_mastellone
    else:
        total_litros_ingresados = total_litros_ingresados_sistema
    ...
    # ==========================================
    # FUNCIONES DE EXPORTACIÓN (ÚNICO PDF)
    # ==========================================
    def generar_pdf(dataframe_gerencia, titulo_dinamico, lit_ingresados, rend_gerencia, ratio_pond_gerenc, df_prod_raw_gerencia):
    ...
    if len(df_consolidado) > 0:
        ...
        pdf_bytes = generar_pdf(df_consolidado, titulo_pdf, total_litros_ingresados, rendimiento_ingreso, ratio_ponderado, df_gerencia_raw_pdf)
